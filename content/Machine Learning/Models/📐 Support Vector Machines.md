If the data is linearly separable, there are infinitely many valid separators. The issue is that [[👓 Perceptrons]] just stumble onto whichever one its update path leads to, which ins't optimal.
...
Formally put, the optimization problem becomes a matter of $\min_{w, b}||w||^2$ subject to $y^i(w*x^i + b) \geq 1$ for all $i=1,...,n$, or in other words, every single trainig point is on the correct side of the boundary with a margin of at least 1.
# Soft-margin SVM
